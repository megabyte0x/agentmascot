...
![Wispr Flow](<Base64-Image-Removed>)![GrowthX](<Base64-Image-Removed>)![Linkup](<Base64-Image-Removed>)![Cloudflare](<Base64-Image-Removed>)

![Worlds largest Hermes Agent buildathon by GrowthX](<Base64-Image-Removed>)

Start →

![Convex](<Base64-Image-Removed>)![OpenAI](<Base64-Image-Removed>)![ElevenLabs](<Base64-Image-Removed>)![Dodo Payments](<Base64-Image-Removed>)
...
OpenAI, Cloudflare, LinkUp, ElevenLabs, Convex, Wispr, GrowthX membership, Dodo. Full list on the Prizes page.
...
Every accepted builder

$400+$300+ before writing a line of code.

You walk out with this regardless. Claim steps on the Prizes page.
...
## Every accepted builder gets $400+$300+ before writing a line of code
...
$200 credits + 1 month Codex Pro ($200 plan)
...
No org ID at registration, no perk. It cannot be issued or reassigned later. This sits with OpenAI, not us.
...
1. Sign up at LinkUp.
2. Go to Settings and open Add Credits.
3. Select $50 and enter code `HERMES`.

[Open LinkUp →](https://linkup.so/)
...
[Claim 3 months free →](http://wisprflow.ai/?promo_code=WISPRXHERMES&months=3)
...
2. Head to the `#│coupon-codes` channel and click **Start Redemption**.
3. Select the event **GrowthX Hackathon** and fill the form with your registered email.
4. The bot sends your unique coupon code.

[Join the Discord →](https://discord.com/invite/VnBvbbcdEC) [Watch the walkthrough](https://youtu.be/S143_JtCtV8)

Dodo Payments

Zero fees on your first $1,000

Your unique code lands in your inbox. Look for the subject line **"Your Dodo Payments perk — GrowthX x Hermes Buildathon"**.

1. Sign up on Dodo Payments.
2. Open Settings and go to Promotions.
3. Enter your code.

[Open Dodo promotions →](https://app.dodopayments.com/settings?tab=promotions)
...
_Available to the winning team in each of the six Indian cities._
...
Your track's rubric below, scored L1 to L5 across its parameters.

Overflow

Uncapped

Past the L5 ceiling on flagged parameters, every additional increment keeps paying.

Power-ups

+25 each

Flat 25 per partner, no cap, listed after each track. All six = 150.All five = 125.

Cross-track bonus

50 cap

Wins in someone else's track at half weight.

Track 01 · 164 base + overflow
...
Max plausible: 1 visitor per 10 weighted impressions. Breach it and the visitors parameter drops to L1 (0 pts) unless the team proves a verifiable non-social traffic source.

Visitors → signups

50% conv. max

Max plausible: 1 signup per 2 visitors. Breach it and the signups parameter drops to L1 (0 pts) unless the team proves a verifiable direct-share source.

### Power-ups on this track.

Do the integration, earn the points: **+25 per partner, no cap, all six = +150all five = +125**. Real use only, a mentor has to see it working in your build.
...
Observability is tool-agnostic
...
Sent us your OpenAI org ID? You already have **$200 in credits** and **Codex Pro (the $200 plan)** on us.

[openai.com →](https://growthx.cc/Openaihermes)

#### OpenRouter / $10 credit

The cheapest way in. Add $10 of credit and point Hermes at it. Same models, more flexibility.

Watch the walkthrough below if you go this route.
...
Pick this if you want tools working without wiring them yourself.

[portal.nousresearch.com →](https://portal.nousresearch.com/)

To run OpenAI, grab a key from [platform.openai.com/api-keys](https://platform.openai.com/api-keys) and put it in `~/.hermes/.env`:
...
**Your provider is not scored.** No track gives points for which model you run. We recommend OpenAI because it is the strongest model driving Hermes today and the credits are already yours. Power-up points come from the sixfive partner integrations only.


Going the OpenRouter route instead? Watch the walkthrough. It covers adding $10 of credit and connecting it to Hermes.

[Video: OpenRouter credit setup →](https://drive.google.com/file/d/1oazfJIQZLO_MFTBB4uSLpko81abfHjf1/view)
...
Hermes reads your repo's `CLAUDE.md` on its own. Nothing to do. It looks for `.hermes.md`, then `AGENTS.md`, then `CLAUDE.md`, and loads **only the first one it finds**. So if your repo already has an `AGENTS.md`, that wins and your `CLAUDE.md` is ignored. Keep one file, not three.

Skills

One line

Point Hermes at your existing skills folder in `~/.hermes/config.yaml` and every one of them shows up as a `/slash` command.

Global instructions

Port by hand

Hermes deliberately does **not** read your global `~/.claude/CLAUDE.md`. Move the durable parts into `~/.hermes/SOUL.md` (how you want it to behave) and `~/.hermes/memories/MEMORY.md` (facts). MEMORY.md is capped at ~2,200 characters, so keep it tight.

Commands and subagents

Rewrite as skills

There is no `.claude/commands` or `.claude/agents` equivalent. Each command becomes a skill, which gives you the same `/name` back. For subagents, Hermes spawns them at runtime instead.

MCP servers

Translate to YAML

Same servers, different file. Your `mcpServers` JSON becomes an `mcp_servers:` block in `~/.hermes/config.yaml`.
...
**There is no one-click importer.**`hermes import` restores Hermes backups, not Claude setups. The prompt above is the fastest honest path. Run it before 10:00 AM, then check `hermes skills browse` shows what you expect.
...
**Create a Telegram bot.** Open Telegram, talk to @BotFather, and send `/newbot`. It asks for a display name and a username ending in `bot`. When it replies with a token, save it somewhere private. Useful later: `/mybots` to inspect your bots, `/setprivacy` for groups, `/revoke` if a token leaks.


Step 2

**Get your numeric Telegram user ID.** Use @userinfobot or @get\_id\_bot. The number matters. Your username does not.


Step 3

**Configure the Hermes gateway.** Run `hermes gateway setup`. Select Telegram. Paste the bot token and your numeric allowed user ID.


Step 4

**Start the gateway and test it.** Run `hermes gateway` and leave it running. Then DM your bot: "Hello Hermes. Reply in one sentence and tell me what tools are active."
...
03Memory and skills (after chat works).
...
`USER.md` \+ `MEMORY.md`, injected into the system prompt on every turn.

Tier 1

Session history

Raw sessions as .jsonl, indexed in SQLite with FTS5 full-text search for cross-session recall.

Tier 2

Pluggable

Holographic (local, free) or Honcho (self-improving user model). Only one active at a time.

Wire up a memory provider, then browse the skills library:
...
[Video: Memory setup, Holographic and Honcho →](https://drive.google.com/file/d/1Mh--j7BB_t6KL0KAyAAyyotIasidHnnu/view?usp=drive_link)

04Checkpoint: is your agent alive?
...
OpenAI key set but Hermes still will not start
...
Telegram bot token copied wrong
...
Telegram user ID is wrong

Get the numeric ID from @userinfobot or @get\_id\_bot, then re-run gateway setup.

```
hermes gateway setup
```

Group messages don't appear
...
Vision over SSH doesn't see clipboard
...
Hermes requires at least 64K tokens of context. Ollama's default is much lower. Set num\_ctx to at least 65536 server-side via a Modelfile.
...
## Bring your own idea or pick one of ours.

**Fresh builds only.** Any idea qualifies, not just this library. But it cannot be a product you were already working on: your company's product, your existing side project, anything with a head start does not apply. The build starts today.

**It must live in one of the three tracks.** Virality, Revenue, or AI as Agency. You are judged on your track's rubric, with partner power-ups on top. More on this in the Scoring section.

Track

AllViralityRevenueAI as Agency

93 of 93 ideasSort: Easy → Hard

EasyVirality

### AI Founder Archetype Quiz

A quiz that labels founders as categories like Distribution Demon, Infra Monk, Demo Goblin, Enterprise Therapist.

Convex · CloudflareOpen the brief ›

EasyVirality

### Am I Underpaid

Drop your role, city, and rate and get a percentile card against live market data: you charge less than 78% of React freelancers in Bangalore. Every anonymous submission sharpens the pool, and the card ends with the one number to quote in your next negotiation.

Linkup · Convex · CloudflareOpen the brief ›

EasyVirality

### Brainrot Rehab

Forward your screen time screenshot and the memes you cannot stop quoting; it stages your brainrot from 1 to 4, then runs your public detox: daily check-ins, a clean-streak badge that updates itself, and a relapse confession card when you fold at 1am.

Convex · Cloudflare · ElevenLabsOpen the brief ›

EasyRevenue

### Cold Email Rewrite Desk

Paste bad sales email. Hermes rewrites it for a specific buyer and creates 3 follow-ups.

Dodo Payments · Linkup · ConvexOpen the brief ›

EasyVirality

### Fine Print Rage Meter

Users paste any terms of service or privacy policy and get the five most outrageous clauses translated into plain English with a 0 to 100 rage score.

Convex · Cloudflare · LinkupOpen the brief ›

EasyRevenue

### Founder Investor Update Writer

Founders paste metrics and notes. Hermes writes investor update, asks, risks, and next month plan.

Dodo Payments · Convex · CloudflareOpen the brief ›

EasyRevenue

### Google Review Reply Desk

Small businesses paste their Google reviews. Hermes drafts replies, apology notes, and recovery offers.

Dodo Payments · Convex · LinkupOpen the brief ›

EasyVirality

### Haggle Score

A chat game where you haggle with an AI bazaar shopkeeper over a real item and get a shareable Haggle Score card with a beat-my-price challenge link.

ElevenLabs · Convex · CloudflareOpen the brief ›

EasyVirality

### Latte Into Lambo

Users paste a past purchase and its date, like 'iPhone 13, Sept 2021', and get a card showing what that money would be worth today in NVDA, BTC, or an index fund.
...
A daily market briefing rendered as one beautiful shareable card: index moves, the three stories that actually moved prices, and one contrarian read, generated as a PNG and pushed to Telegram and WhatsApp channels before market open.

Linkup · Cloudflare · Convex · ElevenLabsOpen the brief ›

EasyVirality

### Meet in the Middle

A WhatsApp bot that ends the where-do-we-meet thread: everyone in the group drops a location pin, it computes the fair midpoint and pitches three venues with honest reasons, and the group votes right in chat.
...
Go NPC for a day: your agent makes every decision, what to eat, when to work, whether to reply to that text, delivered as Telegram prompts you simply obey. Your day streams to a public NPC Mode log page, and you end with a recap card scoring your obedience.
...
A WhatsApp-native CRM for freelancers: text it after every call, like met Priya, wants a logo, follow up Tuesday. The agent maintains the pipeline, nudges you before deals go cold, and drafts the follow-up message for each stale lead.

Convex · Dodo Payments · ElevenLabsOpen the brief ›

EasyRevenue

### Real Estate Listing Optimizer

Brokers paste property details. Hermes creates premium listing copy, WhatsApp messages, photo checklist, and buyer FAQ.
...
A public URL where founders paste a site and get a brutal, funny, useful teardown with share cards.

Convex · Cloudflare · ElevenLabs · LinkupOpen the brief ›

EasyVirality

### Startup Idea Dating Profile

Turn any startup idea into a dating profile: green flags, red flags, toxic traits, best match customers.
...
Paste an X profile and get a persona card, content gaps, best next 10 posts, and shareable score.

Convex · Linkup · CloudflareOpen the brief ›

MediumVirality

### AI Event Matchmaker

Attendees answer 5 questions. Hermes recommends who to meet at the event and why.
...
Input company and competitors. Hermes creates battlecards for sales calls.
...
Fill a 60 second form about your friend and get a personalized birthday anthem performed by voice on a shareable gift page with lyrics and cover art.
...
Connect GitHub commits or build notes. Hermes writes daily build-in-public updates.

Cloudflare · ConvexOpen the brief ›

MediumVirality

### City Roast Map

A playful map that roasts neighborhoods using local signals and memes.

Linkup · Cloudflare · ConvexOpen the brief ›

MediumVirality

### Clicky for Hinge

A real-time wingman for your Hinge conversations. Paste the thread or a screenshot and get three ranked replies: an opener from their profile, a reply that keeps the thread alive, or a save when it goes dry.

Convex · Cloudflare · Dodo PaymentsOpen the brief ›

MediumVirality

### Commentary Box

Upload any short clip, your gully cricket match, your demo, your friend's wedding entry, pick a language and commentator energy, and an agent booth scripts and voices full broadcast commentary over it in minutes.

ElevenLabs · Cloudflare · ConvexOpen the brief ›
...
A city transit bot: subscribe your daily route in one message and get live departures, disruption alerts before you leave the house, and reroutes when things break. Every alert is a clean shareable card your whole office group forwards.

Cloudflare · Convex · LinkupOpen the brief ›

MediumVirality

### Compatibility Duet

One person takes a compatibility interview, then sends a link that tags a partner or crush; the joint verdict card unlocks only when both sides finish.
...
Upload a contract. Hermes produces a business-friendly red flag memo and negotiation email.
...
Upload course notes. Hermes generates quizzes, flashcards, summaries, and practice tests.
...
Upload tickets. Hermes builds macros, escalation tree, FAQ, and response quality rubric.
...
Adult children subscribe; the agent makes a warm daily check-in with their parent on WhatsApp in their language, remembers their stories, and sends the child a short evening digest with anything worth worrying about.

Dodo Payments · ElevenLabs · Convex · CloudflareOpen the brief ›

MediumAI as Agency

### Deal Desk Agency

Upload proposal notes. Hermes creates quote, scope, risk memo, negotiation lines, and handoff docs.

Dodo Payments · ConvexOpen the brief ›

MediumVirality

### Dies at User 50

Drop your vibe-coded app's URL and a swarm of AI test users piles in one at a time until something breaks. You get the Survived User 50 badge or a death certificate reading Died at user 37, cause: unhandled error at checkout, plus a public leaderboard of every app tested.
...
Convex · LinkupOpen the brief ›

MediumVirality

### Flex Card Forge

Users connect a public profile like GitHub, Strava, or chess.com and get a holographic trading card of themselves with a rarity tier earned from real percentile stats.
...
Users answer a five minute chat interview and get a beautiful public date-me page with an AI wingman that answers visitor questions in their voice.
...
An instant-quote lead magnet for local services like painters, movers, and interior work: a homeowner answers five questions and uploads photos, gets a ballpark quote in 60 seconds, and the business gets a qualified lead with photos, scope, and a drafted callback.
...
Users drop links to profiles. Tool creates a digital twin quiz others can take.
...
Upload invoice list. Hermes writes polite follow-ups, tracks status, and drafts WhatsApp/email reminders.
...
Upload docs, transcripts, and FAQs. Hermes builds a knowledge base and Q&A bot.
...
Audit site, rewrite copy, design section order, create test plan, and deploy preview.

Cloudflare · Linkup · ConvexOpen the brief ›

MediumRevenue

### LinkedIn Lead List plus First Lines

Founder describes ICP. Hermes finds leads, writes first-line personalization, and exports CSV.
...
Upload meeting notes. Hermes extracts billable tasks, drafts invoice, and follow-up email.
...
Paste a job description and your resume; a voice interviewer runs a 20 minute adaptive mock, pushes back on weak answers, then replays your three worst fumbles and re-drills each until you pass.

ElevenLabs · Dodo Payments · Linkup · ConvexOpen the brief ›

MediumVirality

### Murder Mystery Game Master

A bot you add to any group chat that hosts a full murder mystery night: secret roles by DM, voiced suspects to interrogate, timed clue drops, and a dramatic reveal.
...
A public feed where your personal AI agent posts one short daily observation about its human: your wins, your 2am commits, your dashboard refreshing habit. You opt in, it pays attention, and its posts are eerily specific.
...
An autonomous newsroom that ships broadcast-quality audio bulletins from live Indian feeds with no human trigger: a monitor watches the wires, an editor picks the rundown, a writer scripts it, a judge fact-gates every line, anchor voices read it, and a publisher posts to a public feed and Telegram channel.

ElevenLabs · Cloudflare · Convex · LinkupOpen the brief ›

MediumRevenue

### Paid Ad Account Audit

Upload exported ad data. Hermes finds waste, bad creatives, audience issues, and next tests.
...
Small claims court for petty fights: both parties submit their side and evidence, an AI judge cross-examines each separately, then delivers a spoken verdict and a shareable ruling card. Every ruling becomes public precedent that future litigants can cite.

ElevenLabs · Convex · Cloudflare · Dodo PaymentsOpen the brief ›

MediumVirality

### Pitch Deck Brutality Score

Upload a pitch deck and get a savage investor-style memo with red flags, missing proof, and a shareable grade.

Linkup · Convex · Dodo PaymentsOpen the brief ›

MediumVirality

### Portfolio Duel

Two friends drop their portfolio or GitHub links into a versus arena and get a head-to-head scorecard that crowns a winner per category, with a challenge link to keep the chain going.

Convex · ElevenLabs · CloudflareOpen the brief ›

MediumVirality

### Price My Room

Upload one photo of your room and get it back with a price tag floating on everything, laptop, chair, the guitar you never play, plus the total: your room liquidates for 2.1 lakh rupees. Priced from live resale listings, not guesses.

Linkup · Convex · Cloudflare · Dodo PaymentsOpen the brief ›

MediumRevenue

### Product Shot Studio

Small sellers send one phone photo of their product to a WhatsApp or Telegram bot and get back a marketplace-ready pack in minutes: clean cutout, lifestyle scene, and an ad creative with copy, paid per pack.
...
Users start at any topic and race down a chain of connected facts, each wilder than the last; when they quit, their rabbit hole path becomes a share card with a depth score.
...
Pick a government procedure, PAN correction, passport renewal, driving licence, and get a personalized step-by-step built from the live portals: exact documents, fees, timelines, the traps, plus WhatsApp nudges that walk you through to done.

Dodo Payments · Cloudflare · Convex · LinkupOpen the brief ›

MediumRevenue

### Renewal Guard

Forward any receipt, invoice, or subscription email to your Renewal Guard address and it builds your vault: every warranty and auto-renewal tracked, with WhatsApp pings before money leaks and a claims walkthrough when something breaks.
...
Paste a freelance gig, client email, or job offer and an agent crew cross-checks the client, payment terms, and known scam patterns, returning a 0 to 100 red-flag score with receipts and a shareable scorecard.
...
Upload a wedding guest list and the agent works it on WhatsApp: confirms attendance, headcounts, meal preferences, flags surprise plus-ones, and gives the family a live tracker.
...
Paste product URL. Get better title, bullets, FAQ, trust copy, images-to-add, and SEO fields.

Dodo Payments · Linkup · Convex · CloudflareOpen the brief ›

MediumVirality

### Situationship: The Trailer

Paste a dead situationship thread and get a 40 second movie trailer: a genre verdict (tragedy, comedy, slow-burn horror), a dramatic voiceover built around your real quotes, a title card, and the tag In Theaters Never. The other person stays anonymous.
...
For an event or community: sponsor list, custom pitch, deliverables, pricing, and follow-up emails.

Linkup · ConvexOpen the brief ›

MediumVirality

### Startup Graveyard

Type a startup idea and an agent team digs up every dead company that tried it, then renders a graveyard of funeral cards: name, years lived, funding burned, cause of death, plus a survival verdict for your version.
...
Upload support tickets or chat logs. Hermes builds FAQ, macros, escalation rules, and help center draft.
...
For freelancers and small firms: connect a mailbox and the agent hunts every invoice and receipt PDF, extracts vendor, GST, and amount, categorizes them, chases clients for missing months, and outputs a clean CA-ready packet.

Convex · Dodo Payments · CloudflareOpen the brief ›

MediumRevenue

### Therapist Intake Desk

Therapists send new clients a link before session one; the agent runs a structured intake conversation and delivers a clinician-ready summary with presenting concern, history flags, and risk markers.

Dodo Payments · Convex · ElevenLabs · CloudflareOpen the brief ›

MediumVirality

### Website in a WhatsApp

A bot that turns a chat thread into a live website: text your business details, photos, and a voice note to WhatsApp or Telegram, get a deployed site on your own link in minutes, then edit it forever by just texting changes.

Cloudflare · Convex · Dodo Payments · ElevenLabsOpen the brief ›

MediumRevenue

### Wedding Gallery Concierge

Photographers drop a delivered wedding gallery; the couple and guests text a bot in plain language, all candids of grandma at the haldi, and get the right photos back. Photographers pay per event and resell it as a premium add-on.
...
A simple WhatsApp bot that answers product questions from a catalog sheet and captures orders.

Dodo Payments · Convex · ElevenLabsOpen the brief ›

MediumVirality

### WhatsApp Sticker Pack Generator

Users send a topic and Hermes creates a sticker pack concept, captions, and exportable assets.
...
Founders enter a one-line description of their startup and get a card showing their closest doppelganger company, how much it raised, and exactly how far ahead it is.
...
Tell it which subscription to kill and Hermes walks the entire cancellation maze on your account, counting every click, retention screen, and fake discount, cancels it, and publishes the receipts: 27 clicks, 4 guilt trips, 1 hidden chat requirement. A public Wall of Shame ranks services by cancellation pain.
...
Audit a team's workflows and propose 5 automations with ROI, tools, setup steps, and risks.
...
A public room where users submit products and Hermes moderates a friendly roast battle.
...
Users submit a brag like 'I hit 10k users' plus evidence links, and get a notarized Verified Brag badge with the evidence trail, or a public Unverified stamp if it does not hold up.
...
A bot that turns missed calls or form leads into WhatsApp follow-ups for clinics.
...
An Indian founder enters their company type and the product builds their compliance world: every MCA, GST, and TDS deadline on a living calendar, routine filings pre-drafted, and a WhatsApp ping before each due date with exactly what to sign.

Dodo Payments · Convex · Cloudflare · LinkupOpen the brief ›

HardRevenue

### Data Room Concierge

Founders upload files. Hermes creates investor data room index, missing docs list, and Q&A bot.
...
A prediction-market intelligence desk: parallel agents watch Polymarket order books, breaking news, and big-wallet moves, spot markets whose odds lag reality, and sell the evidence-backed alerts as a subscription feed. Read-only public APIs, no trading account needed.

Linkup · Convex · Dodo Payments · CloudflareOpen the brief ›

HardAI as Agency

### Finance Ops Agency

Invoice collection, vendor tracker, cashflow view, reimbursement prep, and monthly finance memo.
...
An agent crew you point at a drowning inbox: it triages thousands of emails, unsubscribes from junk, archives dead threads, drafts replies for the ones that matter, and logs every action it takes.
...
An agent operations desk for a real open source repo: a triager labels and reproduces issues, a review agent does first-pass PR review, a docs agent patches stale pages, a community agent drafts replies, and a manager routes it all with a judge gating anything before it posts to GitHub.
...
Paste a list of 20 target accounts and an agency of agents ships a personalized microsite for each: a researcher digs the account, a strategist picks the angle, a builder generates the page in their language, QA screenshots and gates, and a deployer pushes each to its own live URL.
...
An AI on-call engineer: point it at a broken deploy and it reads logs, names the root cause, patches config or code, redeploys, verifies the fix with a real request, and opens the PR, narrated over voice.
...
Drop a podcast episode and an agent crew ships it: a transcriber, a moment scorer, parallel clip cutters, a captioner, a thumbnail artist, and a publisher that posts the top clips to the show's real social accounts.
...
JD rewrite, sourcing strategy, screening rubric, first outreach, interview plan, candidate summary template.
...
ICP, account list, lead enrichment, email sequence, call script, CRM import file.
...
Review repo/docs for security posture, secret exposure, risky dependencies, and missing controls.
...
A full SEO agency as an agent crew: pillar strategy, articles that are not slop, internal links, backlink outreach with real cold emails, Reddit seeding, and AEO. It publishes, promotes, and wins links, not just drafts.
...
Paste a tweet. Hermes turns it into a playable mini-game in the browser.

Cloudflare · Convex · ElevenLabsOpen the brief ›

HardAI as Agency

### User Research Agency

Give it a research question and it runs the whole study: a recruiter agent finds and screens respondents, interviewer agents run structured 10 minute interviews in parallel, an analyst clusters the transcripts, and a writer publishes a findings report with real quotes.

ElevenLabs · Convex · Linkup · CloudflareOpen the brief ›

HardAI as Agency

### Winback Agency

A customer winback agency as an agent crew: a segmenter mines a business's lapsed customer list, a copywriter drafts per-segment offers in brand voice, a sender runs the WhatsApp and email campaign, and a reply handler books orders or logs objections.
...
Drop your live URL there before the deadline: [growthx.club/hermes-buildathon/submit](https://growthx.club/hermes-buildathon/submit). That is the only way to submit. No slides, no zip files.
...
| Proof | 1 min | The numbers on screen: your signups dashboard, your Dodo paymentsyour payment dashboard, or the run log with real output. This is where verification happens. If it isn't on screen, it doesn't count. |
...
My build: [one sentence: what it does and who it's for]
My track: [Virality / Revenue / AI as Agency]
My demo flow: [what I'll show live, step by step]
My real numbers: [signups, revenue, tasks shipped, whatever is true]

How I'm scored:
1. My track's rubric only. Each parameter is L1 to L5, and points =
   (level minus 1) x weight. The root parameter is the heaviest:
   signups for Virality and Revenue, working product shipping real
   output for AI as Agency.
2. Proof comes first. A full minute of my slot is numbers on screen.
3. Numbers are verified, not trusted: live database checks, analytics
   cross-checked against signups. I can only claim what I can show.
4. Partner power-ups are a flat +25 each, but only for real use a
   mentor saw working. Tell me to mention mine only if they're real.

Write a tight 4-minute script: 20 seconds of context, the live demo
with one happy path and one edge case, then the proof minute in the
order I should show my numbers. Flag which rubric parameter each beat
earns, and predict the Q&A question about my weakest number.
```
...
**What if it breaks on stage?** Narrate the intended behavior while it's down: "this is where it pulls the record and confirms the order." Recover, or switch to the backup, and move on. Judges have seen hundreds of demos crash. How you recover says more about your build than a run that never wobbles.
...
[Submit your build →](https://built.growthx.club/)
...
Every partner integration earns a flat +25, no cap. Real use only: a mentor watches it doing real work in your build, or it earns nothing. Stack all six and that is +150.Stack all five and that is +125.
...
| 05 | **Brand and name use.** "GrowthX", "Hermes Buildathon", and associated marks are property of their respective owners. Hermes is a product of Nous Research, and its marks belong to Nous Research. |
...
| 12 | **Third-party tools and updates.** Hermes, partner tools, model APIs, and hosting providers have their own terms. Agreeing to those is on you. GrowthX may update these terms before or during the event. |
